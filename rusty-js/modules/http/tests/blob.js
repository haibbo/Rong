...
(async function () {
  try {
    ...
    results.passed += assert(
      emptyBlob.size == 0,
      "Empty Blob should have size 0",
    );
    results.passed += assert(
      emptyBlob.type === "",
      "Empty Blob should have empty type",
    );

    const textBlob = new Blob(["Hello World"], { type: "text/plain" });
    results.passed += assert(
      textBlob.type === "text/plain",
    ...
    try {
      ...
      results.passed += hasTypeError;
    }
    if (!hasTypeError) {
      throw "Non-array parts should throw TypeError";
    }

    // Test invalid MIME type characters
    const invalidChars = String.fromCharCode(0) + "text/plain";
    const invalidMimeBlob = new Blob([], { type: invalidChars });
    results.passed += assert(
    ...
    try {
      new Blob([], "not an object");
    } catch (e) {
      hasOptionsError = e instanceof TypeError;
      results.passed += hasOptionsError;
    }
    if (!hasOptionsError) {
      throw "Invalid options should throw TypeError";
    }

    // Test invalid endings value - should be ignored and use default
    const invalidEndingsBlob = new Blob(["test"], { endings: "invalid" });
    const validEndingsBlob = new Blob(["test"], { endings: "transparent" });
    results.passed += await assertArrayBufferEquals(
      await invalidEndingsBlob.arrayBuffer(),
      await validEndingsBlob.arrayBuffer(),
      "Invalid endings value should be treated as transparent",
    );
    ...
    results.passed += assert(
      invalidTypeBlob.type === "",
      "Invalid MIME type should be converted to empty string",
    );

    const upperCaseTypeBlob = new Blob([], { type: "TEXT/PLAIN" });
    results.passed += assert(
      upperCaseTypeBlob.type === "text/plain",
      "MIME type should be converted to lowercase",
    );
    ...
    results.passed += assert(
      binaryBlob.size == 4,
      "Blob size should reflect binary data size",
    );

    const multiPartBlob = new Blob(["Hello", new Uint8Array([1, 2, 3])]);
    results.passed += assert(
      multiPartBlob.size == 8,
      "Blob size should calculate multiple parts correctly",
    );
    ...
    results.passed += assert(
      slicedBlob.size == 5,
      "Slice should create Blob with correct size",
    );
    const slicedText = await slicedBlob.text();
    results.passed += assert(
      slicedText === "Hello",
      "Slice should contain correct content",
    );
    ...
    results.passed += assert(
      negativeText === "World",
      "Negative index slice should work correctly",
    );

    const emptySlice = originalBlob.slice(5, 1);
    results.passed += assert(
      emptySlice.size == 0,
      "Invalid range slice should create empty Blob",
    );
    ...
    results.passed += assert(
      new Uint8Array(emptyBuffer).length == 0,
      "Empty Blob should have empty arrayBuffer",
    );
    ...
    results.passed += assert(
      ...
      "text should return correct string",
    );

    const emptyText = await emptyBlob.text();
    results.passed += assert(
      emptyText === "",
      "Empty Blob should return empty string",
    );
    ...
    results.passed += assert(
      transparentText === crlfText,
      "transparent mode should preserve original line endings",
    );

    const nativeBlob = new Blob([crlfText], { endings: "native" });
    const nativeText = await nativeBlob.text();
    const expectedLines = crlfText
      .split(/\r\n|\r|\n/)
      .join(process.platform === "win32" ? "\r\n" : "\n");
    results.passed += assert(
      nativeText === expectedLines,
      "native mode should convert to platform-specific line endings",
    );
