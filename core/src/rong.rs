...
impl MessageReceiver {
    fn new(receiver: mpsc::Receiver<TaskMessage>) -> Self {
        Self { receiver }
    }
...
impl<E: JSEngine + 'static> Rong<E> {
    ...
    pub fn shutdown(&self) -> JSResult<()> {
        ...
        for worker in &self.inner.workers {
            let handle = {
                let mut guard = worker.thread_handle.lock().unwrap();
                guard.take()
            };
...
fn ensure_sync_bridge_allowed(api_name: &str) -> JSResult<()> {
    if CURRENT_WORKER_ID.with(|slot| slot.get()).is_some() {
        return Err(HostError::new(
            crate::error::E_INTERNAL,
...
fn initialize_workers<E: JSEngine + 'static>(
    ...
    message_queue_capacity: usize,
    any_worker_idle: Arc<Notify>,
) -> Result<Vec<Worker<E>>, RongBuildError> {
    ...
    for worker_id in 0..worker_count {
        ...
        let thread_worker_span = worker_span.clone();

        let handle = std::thread::spawn(move || {
            let _entered = thread_worker_span.enter();
